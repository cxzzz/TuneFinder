...
public class TuneFinder1 extends PApplet {
	...
	// Audio file divided in trunks of 2048s
...
/*
			if (transcription.isEmpty() || note.charAt(0) != transcription.charAt(transcription.length() - 1)) {
				transcription += note;
			}
		} else {
			transcription = "";
...
*/
			...
			if (transcription.length() == 30) {
				...
				Tune[] topClosest = tuneIndex.findCloest(transcription, 10);
