...
public class TuneFinder extends JFrame
{
    ...
    public void paint(Graphics g)
    {
        ...
        for (int i = 0; i < w; i++) {
        	// divide the total number of samples into drawing w of them.  => 276480 / w samples
        	int signalIndex = (int) map(i, 0, w, 0, numSamples);
        	float val = signal[signalIndex];
        	// h -- bottom of the screen, 0 top of the screen
        	g.drawLine(i, h / 2, i, (int)map(val, min, max, h, 0));
        }
    ...
    public int countZeroCrossings(float[] signal) {
    	int zc = 0;
    	// zc * samplerate/framerate
